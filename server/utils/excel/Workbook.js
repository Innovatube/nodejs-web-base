...
const XlsParse = require("./Xls/XlsParse");
...
class Workbook {
    ...
    static fromFileAsync(path, opts) {
        ...
        const fileExtend = path.split('.').pop();
        opts.fileExtend = fileExtend;
    ...
    _initAsync(data, opts) {
        ...
        this._maxSheetId = 0;
        this._sheets = [];
        let fileIsEncryption = false;
        if (opts.fileExtend && opts.fileExtend === 'xls') {
            data = new XlsParse(data, opts);
        }
        return externals.Promise.resolve()
            ...
            .then(() => {
                fileIsEncryption = encryptor.checkFileEncryption(data);
                if (!fileIsEncryption) return;
                if (fileIsEncryption && !opts.password) {
            ...
            .then(() => {
                return JSZip.loadAsync(data).catch((e) => {
                    if (fileIsEncryption) {
